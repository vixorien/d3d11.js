...
export class Sky
{
	...
	#SHFromCubemap(cubeSRV, isHDR)
	{
		// Grab data from cube map
		let res = cubeSRV.GetResource();
		let [cubeWidth, cubeHeight, mipLevels, arraySize, isCubemap, format, pixels] = TextureUtils.ReadPixelDataFromTexture(this.#d3dDevice, this.#d3dContext, res);
		res.Release();
		...
		for (let face = 0; face < 6; face++)
		{
			for (let y = 0; y < cubeWidth; y++)
			{
				for (let x = 0; x < cubeWidth; x++)
				{
					...
					let color9Dir = this.#CalcSHForDirection(dir, color);
					for (let i = 0; i < 9; i++)
